...
def mergeXLS(dataDrivenFileLoc, inputLocation, outputLocation, outputFileName):
...
def show_value_pdf (ent, dataDrivenFileLoc):
    ...
    m= Message(master=CustomMessage(),
               ...
               anchor='center',
               font='bold',
               text="Merged PDF File placed at: " + str(outputLocation + "/" + outputFileName + ".pdf"))
    m.pack()

def show_value_xls (ent, dataDrivenFileLoc):
    inputLocation = str(ent['inputFolder'].get())
    outputLocation= str(ent['outputFolder'].get())
    outputFileName= str(ent['fileName'].get())

    mergeXLS(dataDrivenFileLoc, inputLocation, outputLocation, outputFileName)
    #print("Output Merged File generated as :" + str(outputLocation + "/" + outputFileName + ".pdf"))
    ...
    m= Message(master=CustomMessage(),
               ...
               anchor='center',
               font='bold',
               text="Merged EXCEL File placed at: " + str(outputLocation + "/" + outputFileName + ".xlsx"))
    m.pack()


def check_empty_xls (ent):
...
def check_empty_pdf(ent):
    ...
    if (len(dataDrivenFileLoc) > 0 and
        ...
        if dataDrivenFileLoc =='':
            ...
            show_value_pdf(ent, dataDrivenFileLoc)
    else:
        messagebox.showwarning("Warning","You must enter all the fields to proceed")
...
def make_form(master):
    ent=dict()
    Label_0 = tk.Label(master, text="MERGE UTILITY", width=20, font=("bold", 30))
    ...
    instruction1 = tk.Label(master, text="Example: C:/MergeFiles/Input", width=30, font=("normal", 8), anchor='w', justify='left')
    ...
    instruction2 = tk.Label(master, text="Example: C:/MergeFiles/Output", width=30, font=("normal", 8), anchor='w',
                            justify='left')
    ...
    button2 = tk.Button(master, text='Merge PDFS', width=20, bg='brown', fg='white', command=lambda:check_empty_pdf(ent))
    button2.place(x=240, y=340)

    button3 = tk.Button(master, text='Merge EXCELS', width=20, bg='brown', fg='white', command=lambda: check_empty_xls(ent))
    button3.place(x=240, y=375)

    header = tk.Label(master, text="CONVERT PDF TO PPT", width=20, font=("bold, underline", 10), anchor='w',
                      justify='left')
    header .place(x=80, y=430)

    inputFile = tk.Label(master, text="Provide PDF File Location", width=20, font=("bold", 10), anchor='w',
                         justify='left')
    inputFile.place(x=80, y=460)
    button4 = tk.Button(text='Browse A File', bg='#9966ff', fg='#ffffff', command=lambda: fileDialog2())
    button4.place(x=240, y=455)

    Label_5 = tk.Label(master, text="PPT File Name", width=20, font=("bold", 10), anchor='w', justify='left')
    Label_5.place(x=80, y=490)
    pptFileName = tk.Entry(master, width=50)
    pptFileName.place(x=240, y=490)
    instruction3 = tk.Label(master, text="Specify Desired Output File Name", width=30, font=("normal", 8), anchor='w',
                            justify='left')
    instruction3.place(x=240, y=510)
    ent['pptFileName'] = pptFileName

    button6 = tk.Button(master, text='Generate PPT', width=20, bg='brown', fg='white', command=lambda: convertPdfToPpt(pptFileName))
    button6.place(x=240, y=540)

    button7 = tk.Button(master, text='QUIT', width=10, bg='brown', fg='white', command=lambda: master.quit())
    button7.place(x=80, y=560)
